...
file=pd.read_csv(ant+'/ND-X-H.csv', sep=',',skiprows=20,header=None)
file2=pd.read_csv( ant+'/ND-X-C.csv', sep=',',skiprows=20,header=None)
print (file.columns) #writes the simulated values in columns
freq_0= file.values[range(0,file.shape[0]-1),0].astype(float)/1e9 #Read in Frequency
hot_X_db = file.values[range(file.shape[0]-1),1].astype(float) #Read in Specta
cold_X_db = file2.values[range(file.shape[0]-1),1].astype(float) #Read in Specta
...
file=pd.read_csv(ant+'/ND-Y-H.csv', sep=',',skiprows=20,header=None)
file2=pd.read_csv(ant+'/ND-Y-C.csv', sep=',',skiprows=20,header=None)
print (file.columns) #writes the simulated values in columns
freq_0= file.values[range(0,file.shape[0]-1),0].astype(float)/1e9 #Read in Frequency
hot_Y_db = file.values[range(file.shape[0]-1),1].astype(float) #Read in Specta
cold_Y_db = file2.values[range(file.shape[0]-1),1].astype(float) #Read in Specta
