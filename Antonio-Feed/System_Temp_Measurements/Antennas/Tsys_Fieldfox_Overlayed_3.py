...
file3=pd.read_csv(ant3+'/X-pol/XH'+nm3+'.csv', sep=',',skiprows=20,header=None)
file33=pd.read_csv(ant3+'/X-pol/XC'+nm3+'.csv', sep=',',skiprows=20,header=None)
...
file3=pd.read_csv(ant3+'/Y-pol/YC'+nm3+'.csv', sep=',',skiprows=20,header=None)
file33=pd.read_csv(ant3+'/Y-pol/YH'+nm3+'.csv', sep=',',skiprows=20,header=None)
print (file2.columns) #writes the simulated values in columns
freq_03= file3.values[range(0, file.shape[0] - 1), 0].astype(float) / 1e9
...
plt.plot(freq_0,t_sys_Y,label='original')
plt.plot(freq_02,t_sys_Y2,label='new tip-link')
plt.plot(freq_03,t_sys_Y3,label='new tip-link + Gold')
plt.ylim(-50,150)
plt.xlim(0.1, 15)
plt.yticks([-50,-25,0,25,50,75,100,125,150],[-50,-25,0,25,50,75,100,125,150])
#plt.xticks([0,100,200,300,400,500,600,700,801],[0,1.5,3,4.5,6,7.5,9,10.5,12])
